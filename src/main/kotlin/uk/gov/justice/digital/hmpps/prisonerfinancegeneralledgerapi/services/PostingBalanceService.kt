...
class PostingBalanceService(
  ...
  private fun compareTimestamps(timestamp1: Instant?, timestamp2: Instant?): Int {
    ...
    return if (timestamp1 > timestamp2) 1
    else -1
  }

  private fun calculateNewBalance(
    type: BalanceCalculationType, posting: PostingEntity,
  ...
  private fun calculateStrategy(
    previousPostingBalance: PostingBalanceEntity? = null,
    previousStatementBalance: StatementBalanceEntity? = null): BalanceCalculationType {
    return when {
      ...
      compareTimestamps(
        ...
        previousStatementBalance?.balanceDateTime) == 1  -> BalanceCalculationType.FromPreviousPostingBalance
      compareTimestamps(
        previousPostingBalance?.postingEntity?.transactionEntity?.timestamp,
        previousStatementBalance?.balanceDateTime) == -1  -> BalanceCalculationType.FromPreviousStatementBalance
